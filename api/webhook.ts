...
bot.action('duration_custom', async (ctx: any) => {
...
async function showWeeklyStats(ctx: any) {
  if (!ctx.user) {
    await ctx.reply('❌ Error: User not found');
    return;
  }
...
bot.action('view_history', async (ctx: any) => {
  await showWorkoutHistory(ctx);
});
...
async function showLeaderboard(ctx: any) {
  if (!ctx.user) {
    await ctx.reply('❌ Error: User not found');
    return;
  }
...
async function showWorkoutHistory(ctx: any) {
  if (!ctx.user) {
    await ctx.reply('❌ Error: User not found');
    return;
  }
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  if (req.method === 'POST') {
    ...
    res.status(200).json({ message: 'Sport Tracker Bot is running!' });
  }
} 